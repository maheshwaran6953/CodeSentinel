...
export class Analysis implements OnModuleInit, OnModuleDestroy {
  ...
  async process(job: Job) {
    ...
    for (const commit of commits) {
      ...
      if (saved.status === 'completed' || saved.status === 'excluded') continue;
      await this.queue.enqueue('commit',`commit-${saved.id}`, { repositoryId: repo.id, sha: commit.sha });
    }
  }
  ...
  async commit(repo: any, sha: string, token: string) {
    ...
    try {
      ...
      await this.db.query(`UPDATE commits SET author=$2,author_github_id=$3,message=$4,committed_at=$5,
        additions=$6,deletions=$7,changed_files=$8 WHERE id=$1`,[record.id,data.commit.author.name,authorId,data.commit.message,committedAt,data.stats.additions,data.stats.deletions,files.length]);
      if (!authored || merge) {
      ...
      for (const file of files) {
        ...
        evidence.push({ path: file.filename, additions: file.additions, deletions: file.deletions, status: file.status,
          excluded: skip, supported: supported(file.filename), patchAvailable: !!file.patch });
      }
      ...
      const measured = evidence.filter(f=>!f.excluded);
      if (!measured.length) {
        await this.db.query("UPDATE commits SET status='excluded',files=$2,reasons=$3,analyzed_at=now() WHERE id=$1",
          [record.id,JSON.stringify(evidence),JSON.stringify(['Only ignored, generated or third-party files changed'])]);
        return;
      }
      ...
      const history = await this.db.query(`SELECT student_id,features FROM (SELECT student_id,features,committed_at,
        row_number() OVER (PARTITION BY student_id ORDER BY committed_at DESC) AS n FROM commits
        WHERE committed_at<$1 AND status='completed' AND flagged=false AND features IS NOT NULL) h WHERE n<=50 ORDER BY committed_at`,[committedAt]);
      let style: any;
      try { style = await this.ast.analyze({ studentId: repo.student_id,files: sources,history }); }
      ...
      const reasons = [...speed.reasons,...(style.flagged ? ['AST coding style differs from the personal baseline; review language evidence'] : [])];
      ...
      const flagged = speed.flagged || style.flagged;
      ...
      await this.db.source.transaction(async tx => {
        ...
        [record.id,style.error || notes.some(n=>n.startsWith('Source not parsed') || n.includes('parse errors')) ? 'partial' : 'completed',JSON.stringify(evidence),diff.slice(0,24000),JSON.stringify(speed),JSON.stringify({...style,notes}),JSON.stringify(style.features),JSON.stringify(score),score.riskScore,score.authenticityScore,flagged,JSON.stringify(reasons),quizStatus,notes.length ? notes.join('; ') : null]);
        await tx.query('INSERT INTO score_history(commit_id,risk_score,authenticity_score,signals,cause) VALUES ($1,$2,$3,$4,$5)',[record.id,score.riskScore,score.authenticityScore,JSON.stringify(score),'analysis']);
      });
  ...
  async generateQuiz(commitId: string) {
    ...
    if (!commit?.flagged || !commit.diff) return;
