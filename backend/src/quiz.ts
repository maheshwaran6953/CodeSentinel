...
export class QuizController {
  ...
  @Get('active') async active(@Req() req: AuthRequest) {
    ...
    const answerMap = Object.fromEntries(answers.map(a => [a.question_id,{ questionId:a.question_id,answerText:a.answer_text,result:a.result,submittedAt:a.submitted_at,isDraft:a.is_draft }]));
  ...
  @Post('questions/:id/draft') async draft(@Param('id',ParseUUIDPipe) id: string,@Req() req: AuthRequest,@Body() body: AnswerDto) {
    ...
    await this.db.query(`INSERT INTO responses(question_id,student_id,answer_text) VALUES ($1,$2,$3)
      ON CONFLICT(question_id) DO UPDATE SET answer_text=excluded.answer_text,updated_at=now()
      WHERE responses.is_draft=true`,[id,req.user.id,body.answerText]);
    return true;
  }
  @Post('questions/:id/answer') async answer(@Param('id',ParseUUIDPipe) id: string,@Req() req: AuthRequest,@Body() body: AnswerDto) {
    ...
    if (existing?.result) return { ...existing.result, submittedAnswer: existing.answer_text };
    const claim = await this.db.query(`INSERT INTO responses(question_id,student_id,answer_text,is_draft,grading_status,submitted_at)
      ...
      WHERE responses.result IS NULL AND (responses.grading_status<>'grading' OR responses.updated_at<now()-interval '2 minutes') RETURNING answer_text`,
    ...
    try {
      ...
      await this.db.source.transaction(async tx => {
        ...
        if (grades.every((g: any)=>g.result)) {
          ...
          const score = combine(commit.velocity?.risk ?? null,commit.stylometry?.risk ?? null,avg);
      ...
      return { ...result, submittedAnswer: claim[0].answer_text };
    } catch (error) {
